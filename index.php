...
function htmlScript() { ?>
        <script type="text/javascript">
            ...
            function pidToHue(pid) {
                ...
                prevHue = (prevHue + 0.6180339887498948)%1;
                pidHues[pid] = prevHue;
                return pidHues[pid];
            }
            function clearPidHues() {
                pidHues = {};
                prevHue = -1;
            }
            ...
            function addNewUser() {
                $("#userSettings")
                    .html(snippetLoader)
                    .load('?/web/settingstab?tab=addUser', function() {
            ...
            function loadDirectory(dirId) {
                loadCentre('?/web/directory?id='+dirId, function() {
            ...
            function loadCentre(url, callback) {
                clearBackground();

                $('.centrePanel')
                    .html(snippetLoader)
                    .load(url, function(d,s,r) {
                        if (s == 'success') {
                            callback.call(this);
                        } else {
...
function webDirectory() {
    ...
    // files
    if (count($files) > 0) {
        »("<table class=`table table-striped table-condensed tracktable` data-sortable>");
...
function htmlUserSettings() {
    ...
    »("<table id=`userstable` class=`table table-striped table-condensed`>
         <thead><tr>
           <th>User</th>
           <th class=`tinyth`>Admin</th>
           <th>Recent Activity</th>
           <th class=`tinyth`>Edit</th>
         </tr></thead>
         <tbody>");

    foreach ($users as $user) {
        $username   = $user['username'];
        $lastLogged = time_elapsed_string('@'.$user['last_login']);
        $isAdmin    = tickCross($user['admin']);
    ...
    »("<div>
         <button type=`button` onclick=`javascript:addNewUser();` class=`btn btn-default btn-s adduserbtn`>Add User</button>
       </div>");

}

function htmlAddUser() {
    »("<form class=`form-horizontal` role=`form` id=`newUserForm` action=`post`>
         <!-- fake fields are a workaround for chrome autofill getting the wrong fields -->
         <input style=`display:none` type=`text` name=`fakeun`/>
         <input style=`display:none` type=`password` name=`fakepw`/>

         <div class=`form-group`>
           <label for=`newUsername` class=`col-sm-2 control-label`>Username</label>
           <div class=`col-sm-10`>
             <input type=`text` class=`form-control` id=`newUsername` name=`newUsername` placeholder=`Username`>
           </div>
         </div>
         <div class=`form-group`>
         ...
         <div class=`form-group`>
           <label for=`newAdmin` class=`col-sm-2 control-label`>Admin</label>
           <div class=`col-sm-10`>
...
function getPlaylists() {
    echoRO( new ResponseObject('playlists') );
}
...
function checkUsernameAvailable() {
    ...
    $isAvailable = !(dbUsernameExists($username));
...
function getPageMap() {
    return array
        ...
        , '/rest/getPlaylists.view'      => 'getPlaylists'
        ...
        , '/ajax/checkuname' => 'checkUsernameAvailable'
...
function processURI() {
    ...
    if (strpos($rawPath, '/?/') !== false) {
        ...
        // fix IE's ?/& conversion in relPath
        $relPath = fixIEquery($relPath);
...
function formatIndexesXML($indexes) {
    $response = new ResponseObject('indexes', array(
          ...
          'lastModified'    => OPT_TEMP_LASTMODIFIED
        ));
...
function isRightFileType($filename) {
    foreach (explode(' ', OPT_ACCEPTED_FILE_TYPES) as $fileType) {
        if (endsWith(strtolower($filename), $fileType))
            return true;
    }
    return false;
}
...
function mp3lib($attachments=false) {
    ...
    if (isset($getid3)) {
        ...
        manual_require_once('lib/GetID3/getid3.php');
        $getid3 = new getID3;
...
function dbCreateTables() {
    ...
    $db->exec('
        CREATE TABLE IF NOT EXISTS tblKVSettings (
            uid           INTEGER,
            key           TEXT NOT NULL,
            value         TEXT NOT NULL
        );');
    ...
    $db->exec('
        ...
        ;');
    $db->exec('
        CREATE VIEW IF NOT EXISTS vwAlbums as
            SELECT
                album
              , artist as firstartist
...
function dbGetSubFiles($id) {
    $db = dbConnect();
    $q=$db->prepare('SELECT * FROM vwTracks WHERE parentid=? ORDER BY album, trackint, filename');
    $q->execute(array($id));
    $data = $q->fetchAll();
    return $data;
}
...
function dbGetUsers() {
    $db = dbConnect();
    $q=$db->prepare('SELECT user_id, username, last_login, admin FROM tblUsers;');
    $q->execute();
    return $q->fetchAll();
}

function dbUsernameExists($uname) {
    $db = dbConnect();
    $q=$db->prepare('SELECT user_id FROM tblUsers WHERE username=?;');
...
function deployGZB64() {
    ...
    foreach($file_gzb64 as $libName => $libFiles) {
        ...
        // write files
        foreach($libFiles as $fname => $data) {
            file_put_contents("$libDir/$fname", gzuncompress(base64_decode($data)));
        }
    }
