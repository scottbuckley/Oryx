...
function test() {
	...
	$f = '/home/scottbuckley/buck.ly/temp/submusic/A Weather/Everyday Balloons/4.Seven Blankets.mp3';

	print_r(getMP3Info($f));
	exit;
	...
	exit;
	echo "MEOOOW";
	$fname = '/home/scottbuckley/buck.ly/temp/submusic/Bade Ghulam Ali Khan/Immortal Ghazals/36. ANDAAZ APNE DEKHTE HAI.mp3';
...
function scanFolder($parentPath, $parentId, $parentName, $isIndex) {
	...
	foreach ($files as $file) {
		if (isRightFileType($file)) {
			...
			dbWriteTempTrack($filePath, $file, $parentId);
			$containsMusic = true;
		}
	}
...
function scanMP3Info() {
	dbBeginTrans();
	$mp3s = dbGetTempTracks(100);
	foreach($mp3s as $mp3) {
		$meta = getMP3Info($mp3);
		dbUpdateTempMP3Info($mp3, $meta);
	}
	dbEndTrans();
}
function getMP3Info($filename) {
	...
	$all['DurationFinal'] = max(intval($all['Length']), intval($all['LengthID3']));
	$all['Track'] = intornull($all['Track']);

	return $all;
}

function getMP3Info_old($filename) {
	$mp3info = array();
	$oReader = new ID3TagsReader();
	$meta = $oReader->getTagsInfo($filename);

	if (is_array($meta)) {
	    $fields = array('Title', 'Album', 'Author', 'Track', 'Length');
	    foreach ($fields as $field) {
	    	if (array_key_exists($field, $meta)) {
	    		$mp3info[$field] = $meta[$field];
	    	} else {
	    		$mp3info[$field] = "";
	    	}
	    }
...
function dbCreateTables() {
	...
	$db->exec('
		CREATE TABLE IF NOT EXISTS tblTempTracks (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			parentid INTEGER NOT NULL,
			title TEXT,
			album TEXT,
...
function dbWriteTempTrack($filePath, $filename, $parentId) {
	$db = dbConnect();
	$q=$db->prepare('INSERT INTO tblTempTracks (fullpath, filename, parentid) VALUES (?, ?, ?)');
	$q->execute(array($filePath, $filename, $parentId));
}

function dbGetTempTracks($count) {
	$db = dbConnect();
	$q=$db->prepare('SELECT fullpath FROM tblTempTracks WHERE TITLE IS NULL LIMIT ?;');
	$q->execute(array($count));
	$data=$q->fetchAll(PDO::FETCH_COLUMN, 0);
	return $data;
}

function dbUpdateTempMP3Info($fullpath, $data) {
	$db = dbConnect();
	$q=$db->prepare('UPDATE tblTempTracks SET title=?,
	                                          album=?,
	                                          artist=?,
	                                          track=?,
		                                      duration=?,
...
function dbGetSubFiles($id) {
	$db = dbConnect();
	$q=$db->prepare('SELECT id, parentid, fullpath, filename, title, artist, album, duration FROM tblTempTracks WHERE parentid=?');
	$q->execute(array($id));
	$data = $q->fetchAll();
	return $data;
}
...
function dbGetMP3filename($id) {
	$db = dbConnect();
	$q=$db->prepare('SELECT fullpath FROM tblTempTracks WHERE id=?');
	$q->execute(array($id));
	$data = $q->fetchAll(PDO::FETCH_COLUMN, 0);
	return $data[0];
}
